...
class FlexibleConsumerModel:
    ...
    def build(self) -> "FlexibleConsumerModel":
        ...
        # Load bounds are written explicitly below because their dual variables
        # are required in Question 1(c)-(f).
        ...
        # PV bounds are also written explicitly below so that their dual
        # variables can be extracted through the Pi attribute.
        ...
        # Short aliases to make the mathematical expressions easier to read.
        p_import = self.var["import"]
        p_export = self.var["export"]
        load = self.var["load"]
        pv = self.var["pv"]
        ...
        m.setObjective(
            gp.quicksum(
                ...
                - (d.energy_price[t] + d.import_tariff) * p_import[t]
                + (d.energy_price[t] - d.export_tariff) * p_export[t]
        ...
        #
        #       import_t + PV_t = load_t + export_t
        #
        self.con["balance"] = m.addConstrs(
            (
                p_import[t] + pv[t] - load[t] - p_export[t] == 0
                for t in T
            ),
            name="balance"
        )
        ...
        # PV availability limit:
        #
        #       PV_t <= PV_t^max
        #
        self.con["pv_upper"] = m.addConstrs(
            (
                pv[t] <= d.pv_available[t]
                for t in T
            ),
            name="pv_upper"
        )
    ...
    def _extract_results(self, status: str) -> Results:
        ...
        # Question 1(e): report utility and procurement cost separately.
        utility = sum(
